...
Default logging bands (editable per facility, never constants in the UI): feed pH 5.8–6.2, RO pH 5.3–6.3, feed mL Early 1800–2880 / Mid 2100–4680 / Late 2100–3800. RO% = runoff / feed × 100. Green in range, orange high, red low.

Chrome: droplet + title + site name (no dropdown) · **SITES** · small clock. Footer: Overview only.

## AROYA (plan only)

v1 does **not** call `https://api.aroya.io`. Facility settings → Integrations stores an optional API key in localStorage and maps facility/room ids. Status is **Not connected** or **Key saved (not pulling yet)**. Endpoint list lives in `src/lib/aroyaClient.ts` (unused at runtime). Swagger: https://api.aroya.io/public_api/swagger/
